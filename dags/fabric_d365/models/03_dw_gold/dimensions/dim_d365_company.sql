WITH ctco AS (
  SELECT
    ...
    ch.IsDelete
  FROM {{ source('fno', 'companyinfo') }}  ch
  LEFT JOIN  {{ source('fno', 'dirpartytable') }} chprty ON ch.recid = chprty.recid
  LEFT JOIN {{ source('fno', 'omhierarchyrelationship') }} h   ON h.childorganization = ch.recid AND h.hierarchytype = 5637145327 ---considering ONLY legal entities hierarchy
  LEFT JOIN {{ source('fno', 'companyinfo') }}  parentc ON h.parentorganization = parentc.recid
  LEFT JOIN  {{ source('fno', 'dirpartytable') }} pprty ON parentc.recid = pprty.recid
  LEFT JOIN {{ source('fno', 'omhierarchytype') }} ht ON h.hierarchytype = ht.recid  ---considering ONLY legal entities hierarchy
