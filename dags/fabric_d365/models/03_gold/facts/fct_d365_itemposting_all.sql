...
with inventpostingmap as (
    select distinct 
        ...
        en.enumitemlabel as inventpostingname,
    ...
    join {{ref('dim_d365_enum')}} en on ip.inventaccounttype = en.enum_item_value and upper(en.enumname) = 'inventpostinginventaccounttype' 
...
, itemposting_item as (
    ...
    select
        ...
        itm_itemcode.item_recid as item_recid,
        ip.inventaccounttype,
    ...
    join {{ref('dim_d365_financialdimensionaccount')}} f on ip.ledgerdimension = f.financialdimensionaccount_recid
    join {{ref('dim_d365_mainaccount')}} m on f.mainaccount = m.mainaccount_recid

    /* ItemCode = 0 - direct link to Item */
    join {{ref('dim_d365_item')}} itm_itemcode on ip.itemrelation = itm_itemcode.itemid 
        and upper(ip.dataareaid) = upper(itm_itemcode.item_dataareaid) 

    join inventpostingmap map on ip.inventaccounttype = map.inventaccounttype
...
, itemposting_group as (
    ...
    select
        ip.recid posting_recid,
        itm_itemgroup.item_recid as item_recid,
        ip.inventaccounttype,
    ...
    /* ItemCode = 1 - Itemgroup link to Item */
    join {{ref('dim_d365_item')}} itm_itemgroup on ip.itemrelation = itm_itemgroup.itemgroupid 
...
, itemposting_all as (
    ...
    select
        ip.recid posting_recid,
        itm_all.item_recid as item_recid,
        ip.inventaccounttype,
    ...
    join {{ref('dim_d365_item')}} itm_all on ip.itemrelation is null 
...
, itemposting_cat as (
    ...
    select
        ip.recid posting_recid,
        itm_cat.item_recid as item_recid,
        ip.inventaccounttype,
    ...
    join {{ref('dim_d365_itemcategory')}} cat on ip.categoryrelation = cat.category_recid
        
    join {{ref('dim_d365_item')}} itm_cat on itm_cat.item_recid = cat.item_recid
...
, itemposting_cat_nonitem as (
    ...
    join {{ref('dim_d365_category')}} cat on ip.categoryrelation = cat.category_recid
...
, itemposting as (
    ...
    select g.*
    from itemposting_group g
    left join itemposting_item i on g.inventaccounttype = i.inventaccounttype
    ...
    select c.*
    from itemposting_cat c
    left join itemposting_item i on c.inventaccounttype = i.inventaccounttype
        and c.item_recid = i.item_recid
        and i.custvendrelation is null
    left join itemposting_group g on c.inventaccounttype = g.inventaccounttype
    ...
    select a.*
    from itemposting_all a
    left join itemposting_item i on a.inventaccounttype = i.inventaccounttype
        and a.item_recid = i.item_recid
        and i.custvendrelation is null
    left join itemposting_group g on a.inventaccounttype = g.inventaccounttype
        and a.item_recid = g.item_recid
        and g.custvendrelation is null
    left join itemposting_cat c on a.inventaccounttype = c.inventaccounttype
